...
import { buildDateSuggestions, validateDateInput } from "../lib/dateParser.js";
...
test("Case 1: month + day (date mode)", () => {
  assert.deepEqual(labels("Mar 6", { mode: "date" }), ["March 6, 2026"]);
});

test("Case 1: month + day (datetime favor start)", () => {
  const out = first("Mar 6", { mode: "datetime", favor: "start" });
  assert.ok(out);
  assert.equal(out.label, "March 6, 2026, start of day (Asia/Dubai)");
...
test("Case 1: month + day (datetime favor end)", () => {
  ...
  assert.ok(out);
  assert.equal(out.label, "March 6, 2026, end of day (Asia/Dubai)");
...
test("Case 1.1: any order and partial month token", () => {
  assert.equal(first("6 mar", { mode: "date" })?.label, "March 6, 2026");
  assert.equal(first("mar 6", { mode: "date" })?.label, "March 6, 2026");

  const out = labels("6 ma", { mode: "date" });
  assert.deepEqual(out, ["March 6, 2026", "May 6, 2026"]);
});

test("Case 1.1: ambiguous partial month tokens ma / ju", () => {
  assert.deepEqual(labels("ma 6", { mode: "date" }), ["March 6, 2026", "May 6, 2026"]);
  assert.deepEqual(labels("ju 6", { mode: "date" }), ["June 6, 2026", "July 6, 2026"]);
});

test("Case 2: time-only 6 pm in datetime mode", () => {
  assert.deepEqual(labels("6 pm", { mode: "datetime" }), ["March 24, 2026, 6:00 PM (Asia/Dubai)"]);
});

test("Case 2: 6:00 returns both AM and PM", () => {
  assert.deepEqual(labels("6:00", { mode: "datetime" }), [
    "March 24, 2026, 6:00 AM (Asia/Dubai)",
    "March 24, 2026, 6:00 PM (Asia/Dubai)",
  ]);
});

test("Case 2: supported forms 6p / 6pm / 18:00", () => {
  assert.equal(first("6p", { mode: "datetime" })?.label, "March 24, 2026, 6:00 PM (Asia/Dubai)");
  assert.equal(first("6pm", { mode: "datetime" })?.label, "March 24, 2026, 6:00 PM (Asia/Dubai)");
  assert.equal(first("18:00", { mode: "datetime" })?.label, "March 24, 2026, 6:00 PM (Asia/Dubai)");
});
...
test("seconds and milliseconds can be enabled", () => {
  assert.deepEqual(labels("6:00:30", { mode: "datetime", allowSeconds: true }), [
    "March 24, 2026, 6:00:30 AM (Asia/Dubai)",
    "March 24, 2026, 6:00:30 PM (Asia/Dubai)",
  ]);
  assert.deepEqual(
    labels("6:00:30.25", { mode: "datetime", allowSeconds: true, allowMilliseconds: true }),
    [
      "March 24, 2026, 6:00:30.250 AM (Asia/Dubai)",
      "March 24, 2026, 6:00:30.250 PM (Asia/Dubai)",
    ],
  );
});
...
test("numeric date ambiguity is controlled by dateOrder and locale", () => {
  ...
  assert.deepEqual(mdy, ["March 6, 2026", "June 3, 2026"]);

  const dmy = labels("3/6", { mode: "date", dateOrder: "DMY" });
  assert.deepEqual(dmy, ["June 3, 2026", "March 6, 2026"]);

  const autoUS = labels("3/6", { mode: "date", dateOrder: "auto", locale: "en-US" });
  assert.deepEqual(autoUS, ["March 6, 2026", "June 3, 2026"]);

  const autoGB = labels("3/6", { mode: "date", dateOrder: "auto", locale: "en-GB" });
  assert.deepEqual(autoGB, ["June 3, 2026", "March 6, 2026"]);
});

test("year handling defaults and year-only behavior", () => {
  assert.equal(first("March 6", { mode: "date" })?.label, "March 6, 2026");

  const start = first("2026", { mode: "datetime", favor: "start" });
  assert.ok(start);
  assert.equal(start.label, "January 1, 2026, start of year (Asia/Dubai)");
  ...
  assert.ok(end);
  assert.equal(end.label, "December 31, 2026, end of year (Asia/Dubai)");
...
test("invalid dates return no options", () => {
  ...
  assert.deepEqual(labels("Feb 29 2024", { mode: "date" }), ["February 29, 2024"]);
});

test("disambiguation: bare 18 is day-of-month and extra tokens are ignored", () => {
  assert.equal(first("18", { mode: "datetime" })?.label, "March 18, 2026, start of day (Asia/Dubai)");
  assert.equal(first("18 18", { mode: "datetime" })?.label, "March 18, 2026, start of day (Asia/Dubai)");
});

test("ranking: explicit time should not produce inferred start/end-of-day options", () => {
  const out = labels("Mar 6 6pm", { mode: "datetime" });
  assert.equal(out[0], "March 6, 2026, 6:00 PM (Asia/Dubai)");
...
test("validateDateInput is independent and UI-agnostic", () => {
  ...
  assert.equal(valid.suggestions[0]?.label, "March 6, 2026");
