 ...
 * @typedef {"inclusive" | "exclusive"} DateBounds
 ...
 * @property {DateBounds} [bounds="inclusive"]
...
/**
 * @param {string} token
 * @returns {Array<{ month: number, partial: boolean, matchPenalty: number }>}
 */
...
/**
 * @param {string} token
 * @returns {Array<{ weekday: number, partial: boolean, matchPenalty: number }>}
 */
...
function parseDateCandidates(tokens, consumedTimeIndices, now, resolvedOrder, timeFavor, dayFavor) {
  ...
  const separated = [];
  /** @type {Array<{ index: number, months: Array<{ month: number, partial: boolean, matchPenalty: number }> }>} */
  const monthWordTokens = [];
  /** @type {Array<{ index: number, value: number }>} */
  const numericTokens = [];
  /** @type {Array<{ index: number, value: number }>} */
  const dayTokens = [];
  /** @type {Array<{ index: number, weekdays: Array<{ weekday: number, partial: boolean, matchPenalty: number }> }>} */
  const weekdayTokens = [];
...
/**
 * @param {DateSuggestion} suggestion
 * @returns {number}
 */
function suggestionComparableValue(suggestion) {
  return suggestion.mode === "date"
    ? Date.UTC(suggestion.year, suggestion.month - 1, suggestion.day)
    : new Date(suggestion.value).getTime();
}

/**
 * @param {DateSuggestion} suggestion
 * @param {number | null} minComparable
 * @param {number | null} maxComparable
 * @param {DateBounds} bounds
 */
...
export function buildDateSuggestions(input, options = {}) {
  ...
  if (
    minComparable !== null &&
    maxComparable !== null &&
    (bounds === "exclusive" ? minComparable >= maxComparable : minComparable > maxComparable)
  ) {
    return [];
  }
  if (!normalizedInput) {
    ...
    return isWithinBounds(defaultSuggestion, minComparable, maxComparable, bounds)
      ? [defaultSuggestion]
      : [];
  }
  ...
  const boundedSuggestions = suggestions.filter((suggestion) =>
    isWithinBounds(suggestion, minComparable, maxComparable, bounds),
  );
